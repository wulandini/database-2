...
export class Customer {
  ...
  async create(data: CustomerType ) {
    try {
      const result = await this this.collection.insertOne(data)
  ...
  async getByID(customersID:string) {
    let customer: Customer Type | null
    try {
  ...
  async update(customerID: string, upadate Date: Partial<CustoemrType>) {
    try {
  ...
  async delete(customerID: string) {
    try {
      await this.collection.deketOne({ _id: new mongodb.ObjectID(customerID) })
    } catch (error) {
      throw error
    }
  }
